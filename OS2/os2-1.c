...
typedef struct {
    ...
    code_tuple *code; //code tuples가 저장되어있는 위치
    struct list_head job, ready, wait; //코드 튜플을 관리하는 list생성
} process; 
...
void input(){
    while(1) {
		...
		//입력 -> 제대로 못받을시에 바로 동적할당 해제하고 while문에서 나감
...
void reverse_output() {
	...
	list_for_each_entry_reverse(data, &Q_JOB, job) {
		...
		for(int i = 0;i<data->code_bytes/2; i++){
			fprintf(stdout, "%d %d\n", data->code[i].op, data->code[i].len); 
		}
	}
}
