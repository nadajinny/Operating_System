...
//cpu가 2개 이상일 경우를 가정하고 있으므로 구조체로 만들어두기 
typedef struct {
	struct list_head ready; 
	process *cur_process;  //cur_process의 pid가 100이면 idle
	int idle_time;      //각 cpu마다 context switching과 idle중일 때에는 작업할 수 없다. 
	int cs_time; 
	int io_time; 
} cpu; 
...
int cpu_free(cpu *c) {
	if((c->cur_process == NULL || c->cur_process->pid == pid_idle)
		&& empty_readyqueue(c) 
		&& c->cs_time == 0
		&& c->io_time == 0
	) return 1; 
